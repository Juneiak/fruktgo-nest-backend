...
Деньги от заказов не выплачиваются магазину сразу. Они накапливаются в **расчётном периоде** (обычно 2-3 недели). В конце периода платформа рассчитывает итоговую сумму: выручка минус комиссия минус штрафы плюс бонусы. Эта сумма переходит в **аккаунт продавца**, откуда он может запросить вывод на банковский счёт.

**Схема:** OrderPayment → SettlementPeriod → SellerAccount → Withdrawal → Банк продавца

### Ключевая логика

**Расчётный период (SettlementPeriod):**
- У каждого магазина свои периоды (одновременно активен только один)
- Длительность: 14-21 день (настраивается)
- Статусы: `ACTIVE` (идёт накопление) → `PENDING_APPROVAL` (на проверке) → `RELEASED` (одобрен)
- В периоде учитывается: оплаты за заказы, возвраты, штрафы, комиссии, бонусы

**Автоматическое закрытие:**
- По истечению срока периода статус автоматически меняется на `PENDING_APPROVAL`
- PlatformStaff проверяет период (нет ли споров, штрафов, долгов)
- После одобрения деньги переходят в `SellerAccount` продавца

**Аккаунт продавца (SellerAccount):**
- Аккумулирует деньги со всех магазинов продавца
- Продавец запрашивает вывод (минимум 1000₽)
- Выплата на банковский счёт в течение 3-5 рабочих дней

**Штрафы и бонусы:**
- Штрафы за нарушения (просрочка, плохое качество) вычитаются из периода
- Бонусы за хорошую работу добавляются
- Всё фиксируется в `Penalty` с привязкой к периоду

### Защита от мошенничества

Платформа может заморозить период (`FROZEN`) при подозрительной активности или массовых спорах. Деньги не выплачиваются до разбирательства. После проверки период либо одобряется, либо корректируется.
