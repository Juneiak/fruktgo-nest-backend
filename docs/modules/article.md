...
> **Модуль:** `src/modules/article`  
> **Назначение:** Управление статьями и контентом платформы (блог, новости, руководства)
...
**Модуль Article** отвечает за систему контента платформы. Позволяет создавать и управлять статьями для разных типов пользователей.

### Основные возможности

- ✅ Создание статей администраторами
- ✅ Разные статусы публикации (черновик, опубликована, скрыта, архив)
- ✅ Таргетирование по аудитории (все, клиенты, продавцы, сотрудники)
...
Статьи используются для:
- **Блога** - полезные статьи о фруктах, рецепты, советы
- **Новостей** - обновления платформы, акции
- **Руководств** - инструкции для продавцов и клиентов
- **Маркетинга** - промо-материалы
...
**Файл:** `article.schema.ts`

```typescript
class Article {
  _id: Types.ObjectId;
  articleId: string;              // Виртуальное поле (_id.toString())
  
  // Автор
  authorType: ArticleAuthorType;  // Тип автора (ADMIN)
  author?: Types.ObjectId;        // Ссылка на автора (refPath)
  
  // Контент
  title: string;                  // Заголовок статьи
  content: string;                // Полный текст статьи
  contentPreview?: string;        // Превью (первые 200 символов)
  ...
  // Публикация
  status: ArticleStatus;          // Статус публикации
  publishedAt?: Date;             // Дата публикации
  
  // Медиа
  articleImage?: Types.ObjectId;  // Ссылка на Image
  
  // Статистика
  viewCount: number;              // Количество просмотров (default: 0)
...
**Примечание:** В будущем можно добавить:
- `SELLER` - статьи от продавцов
- `PLATFORM_EDITOR` - редакторы контента
...
```typescript
enum ArticleStatus {
  DRAFT = 'draft',           // Черновик (не показывается пользователям)
  PUBLISHED = 'published',   // Опубликована (видна целевой аудитории)
  HIDDEN = 'hidden',         // Скрыта (временно убрана из публичного доступа)
  ARCHIVED = 'archived'      // Архивирована (старая, не актуальная)
}
```
...
DRAFT → PUBLISHED → HIDDEN/ARCHIVED
  ↓                     ↓
  └─────────────────────┘ (можно вернуться в DRAFT для редактирования)
```
...
Целевая аудитория статьи (кто может её видеть).

```typescript
enum ArticleTargetAudience {
  ALL = 'all',                      // Для всех пользователей
  SELLERS = 'seller',               // Только для продавцов
  EMPLOYEES = 'employee',           // Только для сотрудников
  CUSTOMERS = 'customer'            // Только для клиентов
}
```

**Использование:**
- `ALL` - общие статьи в блоге, доступные на главной странице
- `SELLERS` - руководства для продавцов, новости для бизнеса
- `EMPLOYEES` - инструкции для сотрудников магазинов
- `CUSTOMERS` - полезные статьи для покупателей

---

### ArticlesTag

Теги для категоризации и фильтрации статей.
...
enum ArticlesTag {
  ...
  PROMO = 'promo',                        // Акции и спецпредложения
...
- Статья "Как выбрать спелое авокадо" → `[TIPS, FRUITS, EXOTIC]`
- Статья "Летние ягоды: польза и рецепты" → `[SEASONAL, HEALTH, RECIPES]`
- Статья "Как создать свой первый магазин" → `[SELLER_GUIDE, NEWS]`
