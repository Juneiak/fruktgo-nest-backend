...
5. [Смешивание партий](#5-смешивание-партий)
6. [Ценообразование](#6-ценообразование)
7. [Единый остаток магазина](#7-единый-остаток-магазина)
8. [Мягкое резервирование](#8-мягкое-резервирование)
9. [Онлайн-заказ: полный цикл](#9-онлайн-заказ-полный-цикл)
10. [Офлайн-продажа](#10-офлайн-продажа)
11. [Конфликт онлайн/офлайн](#11-конфликт-онлайнофлайн)
12. [Приёмка товара](#12-приёмка-товара)
13. [Перемещения между локациями](#13-перемещения-между-локациями)
14. [Возвраты товара](#14-возвраты-товара)
15. [Списания](#15-списания)
16. [Инвентаризация](#16-инвентаризация)
17. [Сроки годности и алерты](#17-сроки-годности-и-алерты)
18. [Роли и доступ](#18-роли-и-доступ)
19. [Сценарии из жизни](#19-сценарии-из-жизни)
...
### Зачем нужно смешивание

В реальности товар из разных партий может физически смешиваться:
- Высыпание в общий ящик
- Перефасовка
- Консолидация мелких остатков
...
│  ┌───────────────────┐    ┌───────────────────┐            │
│  │ Партия A          │    │ Партия B          │            │
│  │ Клубника 3 кг     │    │ Клубника 2 кг     │            │
│  │ Срок: до 10.12    │    │ Срок: до 12.12    │            │
│  │ Свежесть: 5.0     │    │ Свежесть: 7.0     │            │
│  │ Цена: 300₽/кг     │    │ Цена: 280₽/кг     │            │
│  └─────────┬─────────┘    └─────────┬─────────┘            │
│            │                        │                       │
│            └──────────┬─────────────┘                       │
│                       ▼                                     │
...
│  РЕЗУЛЬТАТ:                                                 │
│  ┌───────────────────────────────────────────┐             │
│  │ Смешанная партия MIX-20241206-001         │             │
│  │ Клубника 5 кг                             │             │
│  │ Срок: до 10.12 (минимальный)              │             │
│  │ Свежесть: 5.8 (средневзвешенная)          │             │
│  │ Цена: 292₽/кг (средневзвешенная)          │             │
│  │                                           │             │
│  │ Состав:                                   │             │
│  │ ├── 60% из Партии A                       │             │
│  │ └── 40% из Партии B                       │             │
...
- **PHYSICAL_MIX** — физическое смешивание (высыпали в один ящик)
- **CONSOLIDATION** — консолидация мелких остатков
- **REPACKING** — перефасовка
- **QUALITY_MERGE** — объединение по качеству

### Ограничения

- Можно смешивать только **один и тот же товар**
- Только партии в **одной локации**
- Нельзя смешивать партии с разным статусом (ACTIVE + BLOCKED)
- История состава сохраняется для трассируемости
...
│  1 день                   →    50%                          │
│                                                             │
│  Правила настраиваются для каждой витрины/продавца.        │
...
### Сценарий: Офлайн хочет забрать зарезервированное

```
Остаток магазина:
├── Всего: 20 кг яблок
├── Резерв: 15 кг (заказ #1234)
└── Доступно: 5 кг

Офлайн-покупатель хочет 12 кг.
```
...
│ Вы хотите продать: 12 кг                               │
│ Будет затронут резерв: 7 кг                            │
│                                                         │
│ Затронутые заказы:                                      │
│ • Заказ #1234 (сборка через 20 мин) — 7 кг             │
│                                                         │
│ [Отменить] [Продать частично (5 кг)] [Продать всё]     │
└─────────────────────────────────────────────────────────┘
```

### Варианты действий кассира

**1. Продать частично (5 кг)**
- Покупатель получает 5 кг
- Резерв не затронут
- Все довольны

**2. Продать всё (12 кг)**
- Покупатель получает 12 кг
- Резерв заказа #1234 уменьшается на 7 кг
- Заказ #1234 помечается как "⚠️ Недостаток товара"
- Менеджеру летит уведомление

**3. Отменить**
- Объяснить покупателю ситуацию
- Предложить другой товар или оформить доставку позже

### Что происходит с заказом при конфликте

```
Заказ #1234 после конфликта:
...
**Варианты решения:**
- Предложить меньшее количество (8 кг вместо 15)
- Предложить замену (другой сорт яблок)
- Перенести доставку (ждём следующую поставку)
- Отменить позицию/заказ
