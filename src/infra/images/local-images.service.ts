...
export class LocalImagesService {
  ...
  async getImageBuffer(
    ...
    const file = await this.getImageById(query.imageId);
    const filePath = join(UPLOAD_DIR, query.size || ImageSize.MD, `${file.filename}`);
    
    try {
      return await fs.readFile(filePath);
    } catch (error) {
      throw new NotFoundException(`Изображение размера ${query.size} не найдено`);
    }
  }



  getImageUrl(fileId: string, size: ImageSize): string {
