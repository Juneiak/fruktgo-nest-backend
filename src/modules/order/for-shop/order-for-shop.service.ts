...
import { Order, OrderStatus } from 'src/modules/order/order.schema';
import {
...
@Injectable()
export class OrderForShopService {
  constructor(
  ...
  async getOrders(authedShop: AuthenticatedUser): Promise<OrderForShopPreviewResponseDto[]> {
    ...
    return plainToInstance(OrderForShopPreviewResponseDto, orders,  { excludeExtraneousValues: true });
  }

  
  async getActiveOrders(authedShop: AuthenticatedUser): Promise<OrderForShopPreviewResponseDto[]> {
    // Проверка существования магазина
    const shop = await this.shopModel.findById(new Types.ObjectId(authedShop.id)).select('_id isBlocked verifiedStatus').lean().exec();
    if (!shop) throw new NotFoundException('Магазин не найден');
    ...
    // Преобразование данных для ответа клиенту
    return plainToInstance(OrderForShopPreviewResponseDto, activeOrders, { excludeExtraneousValues: true });
  }
  ...
  async getOrder(
    ...
    // Преобразование данных для ответа клиенту
    return plainToInstance(OrderForShopFullResponseDto, order, { excludeExtraneousValues: true });
  }
  ...
  //   // Проверяем, что заказ подготовлен и готов к отправке
  //   if (order.orderStatus !== OrderStatus.PREPARING) throw new BadRequestException(`Невозможно вызвать курьера для заказа в статусе ${order.orderStatus}. Заказ должен быть в статусе PREPARING.`);
  ...
  //   // Интеграция с API Достависты
  //   try {
  //     // TODO: Реализовать интеграцию с Достависта API
  //     // Параметры запроса к Достависта:
  //     // 1. Адрес магазина (откуда забрать)
  //     // 2. Адрес доставки (куда доставить)
  //     // 3. Время вызова курьера (dto.callAt)
  //     // 4. Информация о заказе (вес, размеры и т.д.)
      
  //     // Пример кода интеграции:
  //     // const dostavistaResponse = await this.dostavistaService.createDeliveryRequest({
  //     //   pickupAddress: shop.address,
  //     //   deliveryAddress: order.delivery.deliveryAddress,
  //     //   scheduledAt: dto.callAt,
  //     //   orderDetails: {
  //     //     orderId: order.orderId,
  //     //     weight: order.products.reduce((total, p) => total + (p.actualQuantity || 0), 0), // Общий вес заказа
  //     //     cost: order.finances.totalSum // Стоимость заказа
  //     //   }
  //     // });
  //     // 
  //     // if (dostavistaResponse.success) {
  //     //   // Сохраняем идентификатор доставки Достависты
  //     //   order.externalDeliveryId = dostavistaResponse.deliveryId;
  //     // } else {
  //     //   throw new Error(`Ошибка при создании заявки на доставку: ${dostavistaResponse.error}`);
  //     // }
      
  //     // Обновляем статус заказа на "Доставляется"
  //     order.orderStatus = OrderStatus.DELIVERING;
  //     order.preparedAt = dto.preparedAt ? new Date(dto.preparedAt) : new Date();
  ...
  //     return this.getOrder(authedShop, orderId);
  //   } catch (error) {
  //     throw new BadRequestException(`Ошибка при вызове курьера: ${error.message}`);
  //   }
  // }
