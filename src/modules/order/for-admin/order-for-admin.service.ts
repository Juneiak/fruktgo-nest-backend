...
import {
  OrderForAdminPreviewResponseDto,
  OrderForAdminFullResponseDto
} from './order-for-admin.dtos';
...
@Injectable()
export class OrderForAdminService {
  constructor(
  ...
  async getOrders(
    ...
    const items = plainToInstance(OrderForAdminPreviewResponseDto, orders, {excludeExtraneousValues: true, exposeDefaultValues: true});
  ...
  private buildOrderFilter(filter: OrderFilterDto = {}): any {
    ...
    if (filter?.customerId) {
      queryFilter['orderedBy.customer'] = new Types.ObjectId(filter.customerId);
    }
    
    // Фильтрация по магазину
    if (filter?.shopId) {
      queryFilter['orderedFrom.shop'] = new Types.ObjectId(filter.shopId);
    }
    
    // Фильтрация по сотруднику
    if (filter?.employeeId) {
      queryFilter['handledBy.employee'] = new Types.ObjectId(filter.employeeId);
    }
    
    // Фильтрация по смене
    if (filter?.shiftId) {
      queryFilter['shift'] = new Types.ObjectId(filter.shiftId);
    }
    ...
    if (filter?.startDate || filter?.endDate) {
      ...
      if (filter?.startDate) {
        queryFilter['orderedAt']['$gte'] = new Date(filter.startDate);
      }
      
      if (filter?.endDate) {
        queryFilter['orderedAt']['$lte'] = new Date(filter.endDate);
      }
    }
  ...
  async getOrder(authedAdmin: AuthenticatedUser, orderId: string): Promise<OrderForAdminFullResponseDto> {
    checkId([orderId]);
    ...
    return plainToInstance(OrderForAdminFullResponseDto, order, {excludeExtraneousValues: true,exposeDefaultValues: true});
  }
  ...
  async getAllCustomersOrders(authedAdmin: AuthenticatedUser, customerId: string, paginationQuery: PaginationQueryDto): Promise<PaginatedResponseDto<OrderForAdminPreviewResponseDto>> {
    const { page = 1, pageSize = 10 } = paginationQuery;
    const skip = (page - 1) * pageSize;
