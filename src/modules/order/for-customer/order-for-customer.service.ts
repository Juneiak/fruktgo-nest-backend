...
@Injectable()
export class OrderForCustomerService {
  constructor(
    ...
    private cartForCustomerService: CartForCustomerService,
  ...
  async createOrder(authedCustomer: AuthenticatedUser, dto: CreateOrderDto): Promise<OrderCreatedResponseDto> {
    ...
    const cart = await this.cartForCustomerService.validateAndUpdateCart(authedCustomer.id);
    ...
    shop.activeOrders.push(newOrder._id);
    await shop.save();
    ...
    await this.cartForCustomerService.unselectShopForCart(authedCustomer);
  ...
  async getFullOrder(authedCustomer: AuthenticatedUser, orderId: string): Promise<OrderForCustomerFullResponseDto> {
    checkId([orderId]);
    ...
    return plainToInstance(OrderForCustomerFullResponseDto, order, { excludeExtraneousValues: true });
  }
  

  async cancelOrder(authedCustomer: AuthenticatedUser, orderId: string, dto: CancelOrderDto): Promise<OrderForCustomerFullResponseDto> {
    checkId([orderId]);
  ...
  async getOrders(authedCustomer: AuthenticatedUser): Promise<OrderForCustomerPreviewResponseDto[]> {
    ...
    return typedOrders.map(order => {
      return plainToInstance(OrderForCustomerPreviewResponseDto, order, { excludeExtraneousValues: true });
    });
  }
  ...
  async getActiveOrders(authedCustomer: AuthenticatedUser): Promise<OrderForCustomerFullResponseDto[]> {
    // Проверка существования клиента
    const customer = await this.customerModel.findById(authedCustomer.id).lean().exec();
    if (!customer) throw new NotFoundException('Клиент не найден');
    ...
    return typedOrders.map(order => {
      return plainToInstance(OrderForCustomerFullResponseDto, {
