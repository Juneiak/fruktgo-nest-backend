...
export class EmployeeSharedService {
  ...
  async changeEmployeeRequestStatusByEmployee(
    ...
    try {
      const updatedRequestToEmployeeId = await session.withTransaction(async () => {
        const foundEmployee = await this.employeeModel.findOne({ telegramId }).session(session).lean({ virtuals: true }).exec();
        if (!foundEmployee) throw new NotFoundException('Сотрудник не найден');
        verifyUserStatus(foundEmployee);

        const requestToEmployee = await this.requestToEmployeeModel.findOne({ _id: new Types.ObjectId(requestToEmployeeId), to: new Types.ObjectId(foundEmployee._id) }).session(session).exec();
        if (!requestToEmployee) throw new NotFoundException('Запрос не найден или он вам не принадлежит');
        ...
        await requestToEmployee.save({ session });

        const isAccepted = newStatus === RequestToEmployeeStatus.ACCEPTED;
        
        const sellerLogText = isAccepted 
          ? `Сотрудник(${foundEmployee._id.toString()}) принял запрос на работу`
          : `Сотрудник(${foundEmployee._id.toString()}) отклонил запрос на работу`;
          
        const employeeLogText = isAccepted 
          ? `Вы приняли запрос от продавца(${requestToEmployee.from._id.toString()}) на работу`
          : `Вы отклонили запрос от продавца(${requestToEmployee.from._id.toString()}) на работу`;

        if (isAccepted) {
          const foundEmployee = await this.employeeModel.findByIdAndUpdate(requestToEmployee.to, { employer: requestToEmployee.from }, { session }).exec();
          if (!foundEmployee) throw new NotFoundException('Сотрудник не найден');
        }

        await this.logsService.addEmployeeLog(
          foundEmployee._id.toString(),
          employeeLogText,
          { logLevel: LogLevel.LOW, forRoles: [UserType.EMPLOYEE], session }
        );

        await this.logsService.addSellerLog(
          requestToEmployee.from._id.toString(),
          sellerLogText,
          { logLevel: LogLevel.LOW, forRoles: [UserType.SELLER], session }
        );
        
        return requestToEmployee._id.toString();
      });

      if (!updatedRequestToEmployeeId) throw new NotFoundException('Не удалось изменить статус запроса');
      return this.getEmployeeRequestById(updatedRequestToEmployeeId);

    } catch (error) {
      if (error instanceof NotFoundException || error instanceof ForbiddenException) throw error;
      console.error('Ошибка при изменении статуса запроса:', error);
      throw new InternalServerErrorException('Не удалось изменить статус запроса');
    } finally {
      session.endSession();
    }
  }
  ...
  async leaveTheEmployer(telegramId: number): Promise<EmployeeResponseDto> {
    ...
    if (employee.isModified()) {
      ...
      await this.logsService.addEmployeeLog(
        ...
        { logLevel: LogLevel.MEDIUM, forRoles: [UserType.EMPLOYEE] }
      );
      await this.logsService.addSellerLog(
        oldEmployerId,
        `Сотрудник(${oldEmployerId}) ушел от работодателя ${oldEmployerId}`,
        { logLevel: LogLevel.MEDIUM, forRoles: [UserType.SELLER] }
      );
      
      if (oldShopId) await this.logsService.addShopLog(
        oldShopId,
        `Сотрудник(${employee._id.toString()}) открепился от магазина, так как сотрудник ушел от работодателя(${oldEmployerId})`,
        { logLevel: LogLevel.MEDIUM, forRoles: [UserType.SHOP, UserType.SELLER] }
      );
    }
  ...
  async updateEmployeeName(telegramId: number, name: string): Promise<EmployeeResponseDto> {
    ...
    await this.logsService.addEmployeeLog(
      employee._id.toString(),
      `Имя изменено с ${oldName} на ${name}`,
      { logLevel: LogLevel.LOW, forRoles: [UserType.EMPLOYEE] }
    );
